...
gamestationupdateurl="https://gamestation-linux.xorhub.com/upgrades"
...
echo "---- gamestation-config.sh ----" >> $log
...
if [ "$command" == "getRootPassword" ]; then
    ...
    if ! /gamestation/scripts/gamestation-encode.sh decode "${ENCPASSWD}"
    then
	exit 1
    fi
    exit 0
...
if [ "$command" == "setRootPassword" ]; then
    ...
    PASSWDENC=$(/gamestation/scripts/gamestation-encode.sh encode "${PASSWD}")
...
then
    echo "auto"
    gamestation-resolution listOutputs
fi
...
if [ "$command" == "canupdate" ];then
	...
	if test -n "${updateurl}"
	then
	    gamestationupdateurl="${updateurl}"
	fi

	#echo "Update url: ${gamestationupdateurl}/${arch}/${updatetype}/last"
	available=`wget -qO- ${gamestationupdateurl}/${arch}/${updatetype}/last/gamestation.version`
	if [[ "$?" != "0" ]];then
	        echo "Unable to access the url" >&2
		exit 2
	fi
	installed=`cat /gamestation/gamestation.version`
...
if [ "$command" == "update" ];then
	/gamestation/scripts/gamestation-upgrade.sh
	exit $?
fi
...
if [[ "$command" == "wifi" ]]; then
	    ...
	    cat > "/var/lib/connman/gamestation_wifi.config" <<EOF
[global]
Name=gamestation

[service_gamestation_default]
Type=wifi
Name=${ssid}
EOF
	    if test "${psk}" != ""
	    then
		echo "Passphrase=${psk}" >> "/var/lib/connman/gamestation_wifi.config"
	    fi
...
if [[ "$command" == "storage" ]]; then
	...
	INTERNAL_DEVICE=$(/gamestation/scripts/gamestation-part.sh share_internal)
	PARTPREFIX=$(/gamestation/scripts/gamestation-part.sh prefix "${INTERNAL_DEVICE}")
...
if [[ "$command" == "forgetBT" ]]; then
   ...
   rm -f /gamestation/share/system/bluetooth/bluetooth.tar
...
if [ "$command" == "tz" ];then
    if test "$mode" == ""
    then
	cat /gamestation/system/resources/tz
    else
