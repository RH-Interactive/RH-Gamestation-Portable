...
IMGFOLDER="/gamestation/share/system/installs"
ACTION=$1
shift
...
disks_to_keep() {
    grep -E '^[^ ]* /boot |^[^ ]* /gamestation/share' /proc/mounts | sed -e s+"^\([^ ]*\) .*$"+'\1'+ |
	while read X
	do
...
do_listArchs() {
	wget -qO - "${gamestationupdateurl}/installs.txt" | sed -e s+'^\([^/]*\)/.*$'+'\1'+ | sort -u
}
...
do_install() {
	...
	# url
	RELATIVPATH=$(wget -qO - "${gamestationupdateurl}/installs.txt" | grep -E "^${INSARCH}/" | head -1)
	FILEBASENAME=$(basename "${RELATIVPATH}")
	INSIMG="${IMGFOLDER}/${INSARCH}/${FILEBASENAME}"
	test -z "${RELATIVPATH}" && return 1
	url="${gamestationupdateurl}/${RELATIVPATH}"
