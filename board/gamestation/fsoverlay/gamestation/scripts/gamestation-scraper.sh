...
do_scrap() {
	...
	do
	    test "${LRDIR}" = "/gamestation/share/roms/${x}" && EXTRAOPT="-mame"
	done
...
(if test -n "${DOSYS}"
 then
     test -d "/gamestation/share/roms/${DOSYS}" && echo "/gamestation/share/roms/${DOSYS}"
 else
     find /gamestation/share/roms -maxdepth 1 -mindepth 1 -type d
 fi) |
    while read RDIR1
    do
	# read the 2 next dir
