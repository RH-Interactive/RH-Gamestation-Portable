...
rs_current() {
    grep -E "^[^ ]* /gamestation/share " /proc/mounts | sed -e s+'^\([^ ]*\) .*$'+'\1'+
}

# hum, i'm very carefull
# and i'm looking only in /media/usb
# while it could be mounting over the gamestation subdirectory or something else
...
rs_fix_min_dates() {
  ...
  # keep this while some people could have some old files while the gamestation got date from 1970 before
  find /gamestation/share ! -newer /tmp/min_timestamp | while read X; do touch "${X}"; done
}
...
rs_sync() {
    ...
    MOUNTPOINT="/var/run/gamestation-sync"
	...
	if ! /gamestation/scripts/gamestation-mount.sh "${FSTYPE}" 1 "${FSDEV}" "${MOUNTPOINT}"
	then
	    return 1
	fi
    fi
	...
	RSYNCOPT="-rpt --exclude system/bluetooth" # exclude bluetooth while it contains : chars not supported on fat32
        rs_fix_min_dates "/gamestation/share" # if fails, will take a longer time
    fi
    if rsync $RSYNCOPT -v --modify-window=2 --delete-during "/gamestation/share/" "${MOUNTDIR}" 2>&1 # modify-window because all file system such as fat32 doesn't have the same time precision
    then
	EXITCODE=0
    fi
