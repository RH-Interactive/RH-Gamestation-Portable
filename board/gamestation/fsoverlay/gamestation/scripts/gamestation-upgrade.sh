...
do_clean() {
    ...
    rm -f "/gamestation/share/system/upgrade/boot.tar.xz"
    rm -f "/gamestation/share/system/upgrade/boot.tar.xz.md5"
}
trap do_clean EXIT
...
getPer() {
    ...
    while true
    do
	CURVAL=$(stat "/gamestation/share/system/upgrade/boot.tar.xz" | grep -E '^[ ]*Size:' | sed -e s+'^[ ]*Size: \([0-9][0-9]*\) .*$'+'\1'+)
...
test -n "${updateurl}" && gamestationupdateurl="${updateurl}"
...
# download directory
mkdir -p /gamestation/share/system/upgrade || exit 1

# custom the url directory
DWD_HTTP_DIR="${gamestationupdateurl}/${arch}/${updatetype}/last"
...
for fs in /gamestation/share /boot
do
...
touch "/gamestation/share/system/upgrade/boot.tar.xz"
getPer "${size}" &
GETPERPID=$!
mkdir -p "/gamestation/share/system/upgrade" || exit 1
curl -sfL "${url}" -o "/gamestation/share/system/upgrade/boot.tar.xz" || exit 1
kill -9 "${GETPERPID}"
GETPERPID=

# try to download an md5 checksum
curl -sfL "${url}.md5" -o "/gamestation/share/system/upgrade/boot.tar.xz.md5"
if test -e "/gamestation/share/system/upgrade/boot.tar.xz.md5"
then
    DISTMD5=$(cat "/gamestation/share/system/upgrade/boot.tar.xz.md5")
    CURRMD5=$(md5sum "/gamestation/share/system/upgrade/boot.tar.xz" | sed -e s+' .*$'++)
...
BOOTFILES="config.txt gamestation-boot.conf"
...
if ! (cd /boot && xz -dc < "/gamestation/share/system/upgrade/boot.tar.xz" | tar xvf -)
then
    exit 1
fi
