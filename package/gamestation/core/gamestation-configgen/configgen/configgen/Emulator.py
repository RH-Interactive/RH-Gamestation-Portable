...
class Emulator():
    ...
    def configure(self, emulator='default', core='default', ratio='auto', netplay=None):
        recalSettings = UnixSettings(gamestationFiles.gamestationConf)
    ...
    def updateShaders(self, shaderSet):
        if shaderSet != None and shaderSet != 'none':
            shaderfile = gamestationFiles.shaderPresetRoot + '/' + shaderSet + '.cfg'
    ...
    def updateDrawFPS(self):
        try:
            esConfig = ET.parse(gamestationFiles.esSettings)
