...
generators = {
    ...
    'kodi': KodiGenerator(),
    'linapple': LinappleGenerator(os.path.join(gamestationFiles.HOME_INIT, '.linapple'),
...
def get_system_config(system):
    systems_default      = yaml.load(file("/gamestation/system/configgen/configgen-defaults.yml", "r"))
    systems_default_arch = yaml.load(file("/gamestation/system/configgen/configgen-defaults-arch.yml", "r"))
    dict_all = {}
    ...
    # it is flat in the gamestation.conf which is easier for the end user, but i prefer not flat in the yml files
