...
class AmiberryGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        retroconfig = UnixSettings(gamestationFiles.amiberryRetroarchCustom, separator=' ')
        if not os.path.exists(dirname(gamestationFiles.amiberryRetroarchCustom)):
            os.makedirs(dirname(gamestationFiles.amiberryRetroarchCustom))

        romIsWhd = self.isWhdFile(rom)
        commandArray = [ gamestationFiles.gamestationBins[system.config['emulator']], "-G" ]
        if not romIsWhd:
	    ...
	    commandArray.append("pandora.floppy_path=/gamestation/share/roms/amiga/" + system.config['core'])
        ...
        for playercontroller, pad in sorted(playersControllers.items()):
            ...
            with open(gamestationFiles.amiberryRetroarchCustom) as infile, open(playerInputFilename, 'w') as outfile:
	        for line in infile:
