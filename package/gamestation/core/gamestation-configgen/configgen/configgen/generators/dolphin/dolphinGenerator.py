...
class DolphinGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        ...
        dolphinSettings = UnixSettings(gamestationFiles.dolphinIni, separator=' ')
	...
	if system.config['showFPS'] == 'true':
        ...
        # update SYSCONF
        try:
            dolphinSYSCONF.update(system.config, gamestationFiles.dolphinSYSCONF, gameResolution)
        except Exception:
            pass # don't fail in case of SYSCONF update

        commandArray = [gamestationFiles.gamestationBins[system.config['emulator']], "-e", rom]
        return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":gamestationFiles.CONF, "XDG_DATA_HOME":gamestationFiles.SAVES})
