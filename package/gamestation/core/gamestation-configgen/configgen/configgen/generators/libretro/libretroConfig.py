...
libretroSettings = UnixSettings(gamestationFiles.retroarchCustom, separator=' ')
coreSettings = UnixSettings(gamestationFiles.retroarchCoreCustom, separator=' ')
...
def createLibretroConfig(system, controllers, rom, bezel, gameResolution):
    ...
    if defined('shaders', gamestationConfig):
        retroarchConfig['video_shader'] = gamestationConfig['shaders']
    ...
    if defined('ratio', gamestationConfig):
        if gamestationConfig['ratio'] in ratioIndexes:
            retroarchConfig['aspect_ratio_index'] = ratioIndexes.index(gamestationConfig['ratio'])
            retroarchConfig['video_aspect_ratio_auto'] = 'false'
        elif gamestationConfig['ratio'] == "custom":
            retroarchConfig['video_aspect_ratio_auto'] = 'false'
        else:
    ...
    if defined('inputdriver', gamestationConfig):
        retroarchConfig['input_joypad_driver'] = gamestationConfig['inputdriver']
    else:
        retroarchConfig['input_joypad_driver'] = 'udev'

    retroarchConfig['savestate_directory'] = gamestationFiles.savesDir + system.name
    retroarchConfig['savefile_directory'] = gamestationFiles.savesDir + system.name
    ...
    if system.isOptSet('retroachievements') and system.getOptBoolean('retroachievements') == True:
        if(system.name in systemToRetroachievements):
            ...
            retroarchConfig['cheevos_username'] = gamestationConfig.get('retroachievements.username', "")
            retroarchConfig['cheevos_password'] = gamestationConfig.get('retroachievements.password', "")
    ...
    if 'netplaymode' in system.config and system.config['netplaymode'] in systemNetplayModes:
        ...
        retroarchConfig['netplay_ip_port']           = gamestationConfig.get('netplay.server.port', "")
        retroarchConfig['netplay_delay_frames']      = gamestationConfig.get('netplay.frames', "")
        retroarchConfig['netplay_nickname']          = gamestationConfig.get('netplay.nick', "")
        ...
        if system.config['netplaymode'] == 'client':
            ...
            retroarchConfig['netplay_ip_address']        = gamestationConfig.get('netplay.server.ip', "")
...
def writeBezelConfig(bezel, retroarchConfig, systemName, rom, gameResolution):
    ...
    overlay_cfg_file  = gamestationFiles.overlayConfigFile
    ...
    overlay_info_file = gamestationFiles.overlayUser + "/" + bezel + "/games/" + rom + ".info"
    overlay_png_file  = gamestationFiles.overlayUser + "/" + bezel + "/games/" + rom + ".png"
    if not (os.path.isfile(overlay_info_file) and os.path.isfile(overlay_png_file)):
        overlay_info_file = gamestationFiles.overlaySystem + "/" + bezel + "/games/" + rom + ".info"
        overlay_png_file  = gamestationFiles.overlaySystem + "/" + bezel + "/games/" + rom + ".png"
        if not (os.path.isfile(overlay_info_file) and os.path.isfile(overlay_png_file)):
            overlay_info_file = gamestationFiles.overlayUser + "/" + bezel + "/systems/" + systemName + ".info"
            overlay_png_file  = gamestationFiles.overlayUser + "/" + bezel + "/systems/" + systemName + ".png"
            if not (os.path.isfile(overlay_info_file) and os.path.isfile(overlay_png_file)):
                overlay_info_file = gamestationFiles.overlaySystem + "/" + bezel + "/systems/" + systemName + ".info"
                overlay_png_file  = gamestationFiles.overlaySystem + "/" + bezel + "/systems/" + systemName + ".png"
                if not (os.path.isfile(overlay_info_file) and os.path.isfile(overlay_png_file)):
                    overlay_info_file = gamestationFiles.overlayUser + "/" + bezel + "/default.info"
                    overlay_png_file  = gamestationFiles.overlayUser + "/" + bezel + "/default.png"
                    if not (os.path.isfile(overlay_info_file) and os.path.isfile(overlay_png_file)):
                        overlay_info_file = gamestationFiles.overlaySystem + "/" + bezel + "/default.info"
                        overlay_png_file  = gamestationFiles.overlaySystem + "/" + bezel + "/default.png"
                        if not (os.path.isfile(overlay_info_file) and os.path.isfile(overlay_png_file)):
                            return
