...
def writeControllersConfig(controllers):
	if os.path.isfile(gamestationFiles.mupenInput):
		os.remove(gamestationFiles.mupenInput)
...
def writeToIni(controller, config):
	Config.read(gamestationFiles.mupenInput)
	...
	# Open file
	cfgfile = open(gamestationFiles.mupenInput,'w+')
