...
class Pcsx2Generator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        ...
        # config files
        configureReg(gamestationFiles.pcsx2ConfigDir)
        configureUI(gamestationFiles.pcsx2ConfigDir, gamestationFiles.BIOS, system.config, gameResolution)
        configureGFX(gamestationFiles.pcsx2ConfigDir, system.config['showFPS'] == 'true')
        configureAudio(gamestationFiles.pcsx2ConfigDir)

        if isAVX2:
            commandArray = [gamestationFiles.gamestationBins['pcsx2_avx2'], rom]
        else:
            commandArray = [gamestationFiles.gamestationBins['pcsx2'], rom]
        ...
        # plugins
        real_pluginsDir = gamestationFiles.pcsx2PluginsDir
        if isAVX2:
            real_pluginsDir = gamestationFiles.pcsx2Avx2PluginsDir
        ...
        # arch
        arch = "x86"
        with open('/gamestation/gamestation.arch', 'r') as content_file:
            arch = content_file.read()

        if arch == "x86":
            return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":gamestationFiles.CONF})
        else:
            return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":gamestationFiles.CONF, "LD_LIBRARY_PATH": "/lib32", "LIBGL_DRIVERS_PATH": "/lib32/dri"})
...
def configureReg(config_directory):
    ...
    f.write("SettingsFolder=/gamestation/share/system/configs/PCSX2/inis\n")
