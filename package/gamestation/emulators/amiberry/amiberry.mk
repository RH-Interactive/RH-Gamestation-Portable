...
ifeq ($(BR2_PACKAGE_GAMESTATION_TARGET_RPI3),y)
	AMIBERRY_GAMESTATION_SYSTEM=rpi3-sdl2
else ifeq ($(BR2_PACKAGE_GAMESTATION_TARGET_RPI2),y)
	AMIBERRY_GAMESTATION_SYSTEM=rpi2-sdl2
else ifeq ($(BR2_PACKAGE_GAMESTATION_TARGET_RPI1),y)
	AMIBERRY_GAMESTATION_SYSTEM=rpi1-sdl2
endif
...
define AMIBERRY_BUILD_CMDS
	$(TARGET_CONFIGURE_OPTS) $(MAKE) \
		...
		-C $(@D) PLATFORM=$(AMIBERRY_GAMESTATION_SYSTEM)
endef

define AMIBERRY_INSTALL_TARGET_CMDS
	$(INSTALL) -D $(@D)/amiberry-$(AMIBERRY_GAMESTATION_SYSTEM) $(TARGET_DIR)/usr/bin/amiberry
        mkdir -p $(TARGET_DIR)/usr/share/amiberry
	ln -sf /gamestation/share/system/configs/amiberry/whdboot $(TARGET_DIR)/usr/share/amiberry/whdboot
        mkdir -p $(TARGET_DIR)/gamestation/share_init/system/configs/amiberry
	cp -pr $(@D)/whdboot $(TARGET_DIR)/gamestation/share_init/system/configs/amiberry/
