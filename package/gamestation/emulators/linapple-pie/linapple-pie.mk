...
ifeq ($(BR2_PACKAGE_GAMESTATION_SYSTEM),y)
LINAPPLE_PIE_CONFDIR = $(TARGET_DIR)/gamestation/share_init/system/.linapple
...
define LINAPPLE_PIE_INSTALL_TARGET_CMDS
	...
	$(SED) "s|^\(\s*\)Slot 6 Directory =.*|\1Slot 6 Directory = /gamestation/share/roms/apple2|g" $(LINAPPLE_PIE_CONFFILE)
	$(SED) "s|^\(\s*\)Save State Directory =.*|\1Save State Directory = /gamestation/share/saves/apple2|g" $(LINAPPLE_PIE_CONFFILE)
	$(SED) "s|^\(\s*\)FTP Local Dir =.*|\1FTP Local Dir = /gamestation/share/roms/apple2|g" $(LINAPPLE_PIE_CONFFILE)
	echo -e "\n##########################################################################" >> $(LINAPPLE_PIE_CONFFILE)
	echo -e "#\tGamestation specific parameters\n" >> $(LINAPPLE_PIE_CONFFILE)
	echo -e "\tGamestationRomDirectory =\t/gamestation/share/roms/apple2" >> $(LINAPPLE_PIE_CONFFILE)
	echo -e "\tGamestationSaveDirectory =\t/gamestation/share/saves/apple2" >> $(LINAPPLE_PIE_CONFFILE)
endef
else
